...
// Convert a mouse event on the canvas to a tile { c, r }, accounting for DRAW_SCALE and DPR.
export function mouseTile(e, canvas, pcx, pcy) {
  ...
  // Undo the world transform: translate(cw/2, ch/2) scale(DRAW_SCALE) translate(-pcx,-pcy)
  const wx = (lx - canvas.width  / 2) / DRAW_SCALE + pcx
  const wy = (ly - canvas.height / 2) / DRAW_SCALE + pcy
